...
same - (1, 1) - (4, 1)
    ...
    public static List<Word> detectAllWordsMe(int[][] wordSearch, String... words) {
        ...
        for (String s : words) {
            ...
            for (int i = 0; i < hor; i++) {
                StringBuilder sb = new StringBuilder();
                for (int[] aCrossword : wordSearch) {
                    sb.append(aCrossword[i]);
                }

                String verLine = sb.toString();
                if (verLine.contains(s)) {
                    Word word = new Word(s);
                    word.setStartPoint(i, verLine.indexOf(s));
                    word.setEndPoint(i, verLine.indexOf(s) + s.length() - 1);

                    result.add(word);
                    continue outer;
                }
                String verReverse = sb.reverse().toString();
                if (verReverse.contains(s)) {
                    Word word = new Word(s);
                    word.setStartPoint(i, ver - verReverse.indexOf(s) - 1);
                    word.setEndPoint(i, ver - s.length() - verReverse.indexOf(s));
